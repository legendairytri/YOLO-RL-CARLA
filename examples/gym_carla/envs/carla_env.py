...
class CarlaEnv(gym.Env):
  ...
  def __init__(self, params):
    ...
    if self.discrete:
      ...
      params['continuous_steer_range'][1]]), dtype=np.float64)  # acc, steer
    observation_space_dict = {
      'camera': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'lidar': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'birdeye': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
      'state': spaces.Box(np.array([-2, -1, -5, 0]), np.array([2, 1, 30, 1]), dtype=np.float64)
      }
    if self.pixor:
      observation_space_dict.update({
        'roadmap': spaces.Box(low=0, high=255, shape=(self.obs_size, self.obs_size, 3), dtype=np.uint8),
        'vh_clas': spaces.Box(low=0, high=1, shape=(self.pixor_size, self.pixor_size, 1), dtype=np.float64),
        'vh_regr': spaces.Box(low=-5, high=5, shape=(self.pixor_size, self.pixor_size, 6), dtype=np.float64),
  ...
  def reset(self, seed=None, options=None):
    ...
    while True:
      ...
      if self.task_mode == 'random':
        ...
        # for a consistent spawn point
        #transform = self.vehicle_spawn_points[1]

        # for a random spawn point
        transform = random.choice(self.vehicle_spawn_points)
      ...
      if self._try_spawn_ego_vehicle_at(transform):
        ...
        #move spectator/camera to car
        spectator = self.world.get_spectator()
        # change z value on next line to move camera higher up
    ...
    self.reset_step+=1
    print("reset_step = " + str(self.reset_step))
  ...
  def step(self, action):
    ...
    self.total_step += 1
    print("total_step = " + str(self.total_step))
  ...
  def _try_spawn_ego_vehicle_at(self, transform):
    ...
    overlap = False
    # if there's overlap, overlap = true
    for idx, poly in self.vehicle_polygons[-1].items():
      ...
      if dis > 8:
        ...
        overlap = True
        print("spawning is encountering overlap")
        break
    ...
    # runs if there isn't overlap
    # skips if there's overlap
    if vehicle is not None:
      self.ego=vehicle
      print("")
      print("successfully respawned")
      return True
      
    # returns false if there's overlap
    return False
  ...
  def _get_obs(self):
    ...
    # Display on pygame
    # comment next line out to prevent pygame window from taking up resources
    pygame.display.flip()
    ...
    if self.pixor:
      obs.update({
        ...
        'vh_clas':np.expand_dims(vh_clas, -1).astype(np.float64),
        'vh_regr':vh_regr.astype(np.float64),
  ...
  def _get_reward(self):
    ...
    r = collision_coef*r_collision + lspeed_lon_coef*lspeed_lon + r_fast_coef*r_fast + r_out_coef*r_out + r_steer_coef*r_steer + r_lat_coef*r_lat - dread
    print()
    print("reward: " + str(r))
    
    print('collision reward: '+ str(collision_coef*r_collision))
    print('longitudinal speed reward: '+ str(lspeed_lon_coef*lspeed_lon))
    print('too fast reward: '+ str(r_fast_coef*r_fast))
    print('out of lane reward: '+ str(r_out_coef*r_out))
    print('steering reward: '+ str(r_steer_coef*r_steer))
    print('lateral acceleration reward: '+ str(r_lat_coef*r_lat))
